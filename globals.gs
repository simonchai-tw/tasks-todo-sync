...
const ALERT_KEYS = {
  ...
  timeBridgeQuarantine: 'alert_tb_quarantine_last_at'
};
let RUN_STARTED_AT = 0;
let SYNC_OBSERVABILITY_ = null;
const MOVE_FINGERPRINT_PREFIX = 'sha256b64:';

// Time Bridge constants (v0.8.0: canonical time record + idempotent renderers)
const TIME_BRIDGE_OPS_MAX_PER_ROUND = 20;
// R-4 failure ladder: full speed for the first failures, then one attempt per
// day; after the slow lane also fails repeatedly the pair is quarantined and the
// operator is notified once per alert cooldown.
const TIME_BRIDGE_RETRY_FULL_SPEED_MAX = 6;
const TIME_BRIDGE_RETRY_SLOW_LANE_MAX = 3;
const TIME_BRIDGE_RETRY_SLOW_LANE_INTERVAL_MS = 24 * 60 * 60 * 1000;
const TIME_BRIDGE_MARKER_REGEX_ = /^[ \t\u3000]*[\[［][ \t\u3000]*TTS-TIME[ \t\u3000]*[:：][ \t\u3000]*(?:([01]\d|2[0-3])[:：]([0-5]\d)|(NONE))[ \t\u3000]*[\]］][ \t\u3000]*\r?$/i;
const TIME_BRIDGE_WHOLE_NOTES_CHECK_REGEX_ = /^[ \t\u3000]*[\[［].*TTS-TIME.*[\]］].*\r?$/i;
const TIME_BRIDGE_EVENT_DURATION_MINUTES = 30;
const TIME_BRIDGE_DEAD_LETTER_EVENT_DELETIONS_MAX = 20;
