...
export function runGasFilesInContext(context, order = GAS_SOURCE_FILES) {
  ...
  // Test default for the pre-PATCH notes re-read (W4 reread).  The harness
  // mirrors the production re-read (a fresh provider GET) WITHOUT any
  // fail-open fallback (WO-9): a missing or throwing provider read propagates
  // to the caller exactly like production surfaces a failed re-read, so any
  // test that reaches this path must stub getGTask_/getMsTask_ (or override
  // rereadProviderNotes_ itself) explicitly.
